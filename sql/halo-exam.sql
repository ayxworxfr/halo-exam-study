...
CREATE TABLE `answer_question`  (
  ...
  `exam_user_id` bigint(20) NULL DEFAULT NULL COMMENT '考试用户表id',
...
CREATE TABLE `exam_user`  (
  ...
  `exam_id` varchar(0) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NULL DEFAULT NULL COMMENT '考试id',
...
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `exam_detail` AS select `exam_user`.`user_id` AS `user_id`,`exam_user`.`exam_id` AS `exam_id`,`exam`.`class_id` AS `class_id`,`exam`.`subject_id` AS `subject_id`,`exam`.`name` AS `exam_name`,count(`user_answer_detail`.`mark_score`) AS `mark_score`,count(`paper_question`.`score`) AS `score`,`exam`.`start_time` AS `start_time`,`exam`.`end_time` AS `end_time` from (((`exam_user` join `user_answer_detail`) join `exam`) join `paper_question`) where ((`exam_user`.`id` = `user_answer_detail`.`exam_user_id`) and (`exam_user`.`exam_id` = convert(`exam`.`id` using utf8mb4)) and (`exam`.`paper_id` = `paper_question`.`paper_id`)) group by `exam_user`.`exam_id`,`exam_user`.`user_id`,`exam`.`class_id`;

-- ----------------------------
-- View structure for user_answer_detail
-- ----------------------------
DROP VIEW IF EXISTS `user_answer_detail`;
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `user_answer_detail` AS select `answer_question`.`exam_user_id` AS `exam_user_id`,`answer_question`.`answer` AS `user_answer`,`answer_question`.`mark_score` AS `mark_score`,`answer_question`.`question_id` AS `question_id`,`question_answer`.`answer` AS `answer`,`question_answer`.`analysis` AS `analysis`,`question_answer`.`score` AS `score`,`answer_question`.`answer_time` AS `answer_time` from (`answer_question` join `question_answer`) where (convert(`answer_question`.`question_id` using utf8mb4) = `question_answer`.`question_id`);
